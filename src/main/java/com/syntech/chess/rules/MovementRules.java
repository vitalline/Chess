...
public class MovementRules {
    public static void addDiagonalMovement(@NotNull Point pos, @NotNull Board board, ArrayList<Move> moves) {
        ...
        while (board.isFree(pos.x + d, pos.y + d)) {
            moves.add(new Move(pos.x + d, pos.y + d));
            d++;
        }
        d = 1;
        while (board.isFree(pos.x + d, pos.y - d)) {
    ...
    public static void addOrthogonalMovement(@NotNull Point pos, @NotNull Board board, ArrayList<Move> moves) {
        ...
        while (board.isFree(pos.x + ++d, pos.y)) {
            moves.add(new Move(pos.x + d, pos.y));
        }
        d = 0;
        while (board.isFree(pos.x + --d, pos.y)) {
            moves.add(new Move(pos.x + d, pos.y));
        ...
        while (board.isFree(pos.x, pos.y + ++d)) {
            moves.add(new Move(pos.x, pos.y + d));
        }
        d = 0;
        while (board.isFree(pos.x, pos.y + --d)) {
            moves.add(new Move(pos.x, pos.y + d));
    ...
    public static void addLeapingMovement(@NotNull Point pos, @NotNull Board board, int x, int y, ArrayList<Move> moves) {
        if (board.isFree(pos.x + x, pos.y + y)) {
            moves.add(new Move(pos.x + x, pos.y + y));
        }
        if (board.isFree(pos.x + y, pos.y + x)) {
            moves.add(new Move(pos.x + y, pos.y + x));
        }
        if (board.isFree(pos.x - x, pos.y + y)) {
            moves.add(new Move(pos.x - x, pos.y + y));
        }
    ...
    public static void addPawnLikeMovement(@NotNull Point pos, @NotNull Board board, int moveDirection, boolean hasNotMoved, ArrayList<Move> moves) {
        ...
        moves.add(new Move(pos.x + moveDirection, pos.y));
        if (hasNotMoved && board.isFree(pos.x + moveDirection * 2, pos.y)) {
            moves.add(new Move(pos.x + moveDirection * 2, pos.y));
        }
    }
    ...
    public static void addPawnLikeThreatening(@NotNull Point pos, @NotNull Board board, Side side, int moveDirection, ArrayList<Move> moves) {
        if (board.getSide(pos.x + moveDirection, pos.y - 1) == side) {
            moves.add(new Move(pos.x + moveDirection, pos.y - 1));
        }
        if (board.getSide(pos.x + moveDirection, pos.y + 1) == side) {
            moves.add(new Move(pos.x + moveDirection, pos.y + 1));
        }
